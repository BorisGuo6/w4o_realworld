...
from anydexgrasp_model import GroundedAnyDexGraspNet
import imageio
...
parser.add_argument('--ckpt_path', type=str, default='/home/boris/workspace/World4Omni/ckpt/checkpoint.tar.18', help='Path to the model checkpoint')
...
def images_to_video(images, video_path, frame_size=(1920, 1080), fps=30):
    ...
    # Use imageio-ffmpeg for MP4 format
...
def main(cfgs):
    ...
    grounded_anydexgrasp = GroundedAnyDexGraspNet(ckpt_path=cfgs.ckpt_path, task_name=task.get_name())
    ...
    goal_action = grounded_anydexgrasp.step(obs, prompt, cam_int, cam2world)
    ...
    for i, pose in enumerate(transitional_poses_list):
        ...
        try:
            ...
            # Gracefully handle planning failures: skip this step and continue
            print(f'Planning failed at step {i}:', e)
            continue


    # Skip video generation for now due to imageio issues
    print(f'Skipping video generation. {len(image_list)} images captured.')
    print('Done')
    env.shutdown()
    end_time = time.time()
