...
if __name__ == '__main__':
    ...
    # 2. (optinal) load camera intrinsics params
    # K_path = CAMERA_ASSETS_PATH / serial_number / "K.npy"
    # K = np.load(K_path)
    ...
    init_X_BaseCamera_path = CAMERA_DATA_PATH / serial_number/ exp_name / "init_X_BaseCamera.npy"
    init_X_BaseCamera = np.load(init_X_BaseCamera_path)
    init_X_CameraBase = np.linalg.inv(init_X_BaseCamera)
    
    # 4. setup the robot arm
    if robot_type == 'xarm6':
        arm = XArm6WOEE()
    elif robot_type == 'xarm7':
        arm = XArm7WOEE()
    
    # 5. load camera extrinsic samples
