...
def compute_similarity_from_matchdict(M, depth1: np.ndarray, depth2: np.ndarray, K: np.ndarray,
    ...
    s_final, R_final, t_final, err, P, Q, Q_hat = Kabsch(pts1, pts2, with_scale=True)
...
if __name__ == "__main__":
    ...
    # Init resized DepthAnything
    ds_p = dpt_ini_resized.flatten()
    ds_p = ds_p[filter]
    xs_p = (us + intrinsics[0,2]) * ds_p / intrinsics[0,0]
    ys_p = (vs + intrinsics[1,2]) * ds_p / intrinsics[1,1]
    zs_p = ds_p
    pts_p = np.stack([xs_p, ys_p, zs_p], axis=1) # [H*W, 3]

    seg_ini = seg_ini.flatten()[filter]
    obj_pts = pts_p[seg_ini>0.5]
    goal_pts = (R@obj_pts.T + t[:, None]).T
    goal_colors = colors[seg_ini>0.5]

    # Goal resize DepthAnything
    grids = np.meshgrid(np.arange(W), np.arange(H)) # [H, W]
    us = grids[0].flatten()
    vs = grids[1].flatten()
    goal_ds = dpt_tgt_resized.flatten()
    ...
    print(goal_gt_obj_pts.mean(axis=0)-obj_pts.mean(axis=0), (Q-P).mean(axis=0))
