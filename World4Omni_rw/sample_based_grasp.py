...
from World4Omni_rw.tools.get_new import get_newest      # [Optional] automatically read newest dir in real world
...
def filter_boundary_points(
    organized_point_cloud: np.ndarray,
    mask_input,
    ...
    # --- 原有逻辑保持不变 ---
    # 1. 输入验证
    if organized_point_cloud.shape[:2] != mask.shape:
        print(f"Warning: 点云形状 {organized_point_cloud.shape[:2]} 和掩码形状 {mask.shape} 不匹配。")
        raise ValueError("点云和掩码的H、W维度必须匹配!")
    if boundary_width <= 0:
        return organized_point_cloud[mask.astype(bool)]
    ...
    # 4. 使用腐蚀后的掩码来筛选点云
    clean_mask_bool = eroded_mask.astype(bool)
    clean_point_cloud = organized_point_cloud[clean_mask_bool]
    
    return clean_point_cloud


def preprocess(scene_points, mask_image, boundary_width: int = 5):
    """
    ...
    # --- 主要修改部分在这里 ---
    # 在展平之前，对2D掩码进行腐蚀操作来“收缩”它
    if boundary_width > 0:
        kernel_size = 2 * boundary_width + 1
        kernel = np.ones((kernel_size, kernel_size), np.uint8)
        
        # 确保掩码是OpenCV期望的uint8格式
        mask_uint8 = (mask_array > 0).astype(np.uint8) * 255
        eroded_mask = cv2.erode(mask_uint8, kernel, iterations=1)
        
        # 后续步骤将使用这个被腐蚀过的掩码
        mask_to_process = eroded_mask
    else:
        # 如果不设置边界宽度，则使用原始掩码
        mask_to_process = mask_array
    # --- 修改结束 ---

    # Flatten the (now smaller) mask to a 1D boolean array
    # True where pixel value is not zero
    flat_mask = mask_to_process.flatten() > 0 # <--- 这里使用了新的mask
...
def sample_grasp(obj_points, num_grasps=200, z_offset=0.06, random_angle_min=-np.pi/4, random_angle_max=np.pi/4):
    """
    ...
    for idx in indices:
        ...
        # random_angle = np.random.uniform(np.pi/4, np.pi/4*3)
        random_angle = np.random.uniform(random_angle_min, random_angle_max)
        # random_angle = np.random.uniform(-np.pi/4, np.pi/4*3)
...
def check_collision(scene_points, grasps, gripper_obj_path, collision_threshold=0):
    """
    Check for collisions between the gripper and the scene for each grasp.
    This version loads a single mesh file for the gripper and checks if the
    number of scene points contained within the gripper mesh exceeds a threshold.
    ...
    Args:
        ...
        collision_threshold (int): The number of points allowed inside the gripper
                                   before it's considered a collision.
    ...
    for grasp_pose in grasps:
        ...
        # Check how many scene points are inside the transformed gripper mesh.
        # The .contains() method returns a boolean array. Summing it gives the
        # total number of points inside.
        points_inside_mask = transformed_gripper.contains(scene_points)
        num_points_inside = np.sum(points_inside_mask)
        
        # A collision is now defined as having more than `collision_threshold` points inside.
        if num_points_inside <= collision_threshold:
            valid_grasps.append(grasp_pose)
...
def main():
    ...
    scene_points_path = f"{RAW_DATA_DIR}/{basename}/pointcloud_init_points.npy"
    scene_point_color_path = f"{RAW_DATA_DIR}/{basename}/pointcloud_init_colors.npy"
    mask_path = f"{RAW_DATA_DIR}/{basename}/mask_init.png"
    ...
    z_offset = 0.14     # z_offset in meters, adjust to get better grasp depth
    random_angle_min = 0  # Control the random angle range for sampling
    random_angle_max = np.pi / 4   # Control the random angle range for sampling
    # angle_thresholds = {    # filter grasps by angle to camera axes
    #     'x': (25, 90),
    #     'y': (30, 90),
    #     'z': (0, 65)
    # }
    angle_thresholds = {    # filter grasps by angle to camera axes (top down)
        'x': (25, 115),
        'y': (0, 60),
        'z': (0, 90)
    }
    ...
    initial_obj_points = preprocess(scene_points, object_mask, boundary_width=5)
    ...
    # obj_points = filter_boundary_points(obj_points, object_mask, boundary_width=5)

    # Sample grasp configurations
    print("2. Sampling grasp configurations...")
    grasps = sample_grasp(obj_points, num_grasps=sample_num, z_offset=z_offset,
                         random_angle_min=random_angle_min, random_angle_max=random_angle_max)
    ...
    valid_grasps = check_collision(scene_points, filtered_grasps, gripper_obj_path, collision_threshold=collision_threshold)
    ...
    save_path = f"{RAW_DATA_DIR}/{basename}/grasps_in_cam.npz"
    if sorted_grasps:
